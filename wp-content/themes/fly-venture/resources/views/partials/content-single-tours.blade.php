@php
  ...
  $rating_value = ! empty($rating['selectRating'])
      ? max(1, min(5, absint($rating['selectRating'])))
      : 0;
...
<section class="{{ $section_classes }}" >
  ...
  <div class="hero-bg">
    ...
    @if ( ! empty( $hero['featuredImageUrl'] ) )
      <img
        src="{{ esc_url( $hero['featuredImageUrl'] ) }}"
        alt="{{ esc_attr( $hero['title'] ?? 'Hero background' ) }}"
        ...
        <div
          ...
          @if ( ! empty( $hero['recommendedText'] ) )
            <div class="flex items-center gap-6">
              <img src="{{ esc_url( $check_green_uri ) }}" alt="{{ esc_attr__( 'recommended', 'fly-venture' ) }}"
                   class="w-16 h-16">
                ...
                @foreach (  $tourSinglePageData['imageSlider'] as $slide )
                  ...
                  @if ( $slideUrl )
                    <div class="swiper-slide">
                      <img src="{{ $slideUrl }}" alt="{{ $slideAlt }}" class="w-full h-full object-cover">
                    </div>
                  @endif
                @endforeach
              </div>
                ...
                @if ( ! empty( $tab['title'] ) )
                  <li>
                    <button
                      type="button"
                      class="tab-btn"
                      data-target="{{ esc_attr( $tabSlugs[ $i ] ) }}"
                    >{{ esc_html( mb_strtoupper( $tab['title'] ) ) }}</button>
                  </li>
                @endif
              @endforeach
            </ul>
          ...
          <div class="tab-content w-full" data-tab="{{ esc_attr( $tabSlugs[ $i ] ) }}">
            ...
            @if ( $tab['isLocation'] && ! empty( $tab['locations'] ) )
              ...
              <div class="content">
                ...
                @endif
                @php
                  $visibleLocations = array_filter( $tab['locations'], fn($loc) =>
                      ! empty( $loc['title'] )
                      || ! empty( $loc['icon']['url'] ?? '' )
                    ...
                    <div class="box">
                      @if ( ! empty( $locTitle ) || $locIconUrl )
                        <div class="title title-black">
                          <h4>
                            @if ( $locIconUrl )
                              <img src="{{ $locIconUrl }}" alt="{{ $locIconAlt }}" height="20" width="20">
                            @endif
                            {{ esc_html( $locTitle ) }}
                          </h4>
                        </div>
                      ...
                      @if ( ! empty( $locDesc ) || ! empty( $locLink['url'] ) )
                        <div class="box-inner">
                          ...
                          @if ( ! empty( $locLink['url'] ) )
                            <a
                              href="{{ esc_url( $locLink['url'] ) }}"
                              class="blue-link"
                              role="link"
                              @if ( ($locLink['target'] ?? '') === '_blank' ) target="_blank"
                              rel="noopener noreferrer" @endif
                            >{{ esc_html( $locLink['title'] ?? __('Get Directions', 'fly-venture') ) }}</a>
                          @endif
                        </div>
                      @endif
                    </div>
                ...
                @elseif ( ! empty( $mapField['lat'] ) && ! empty( $mapField['lng'] ) )
                  ...
                  <div class="map-img mt-24">
                    <iframe
                      ...
                      title="{{ esc_attr( $tab['title'] ?? __('Route Map', 'fly-venture') ) }}"
                    ></iframe>
                  </div>
                @endif
              </div>
      ...
      <div class="lg:col-span-4 w-full lg:sticky top-100 pl-40 max-1441:pl-30 max-1023:pl-0">
        <div
            ...
            @if ( ! empty( $priceBlock['offerPrice'] ) || ! empty( $priceBlock['perPersonText'] ) )
              <div class="flex items-baseline gap-8">
                @if ( ! empty( $priceBlock['offerPrice'] ) )
                  <strong>{{ esc_html__( 'Now', 'fly-venture' ) }} {{ esc_html( $priceBlock['offerPrice'] ) }}</strong>
                @endif
              ...
              <div class="flex flex-wrap gap-x-14 gap-y-0 mt-8">
                ...
                @if (!empty($priceTags))
                    <div class="best-price-tag price-tag-item"
                         style="<?php echo !empty($priceTags[0]['color']) ? 'background-color:' . esc_attr($priceTags[0]['color']) . ';' : '' ?>">

                        <?php if (!empty($priceTags[0]['icon_url'])) : ?>
                      <img
                        src="<?php echo esc_url($priceTags[0]['icon_url']); ?>"
                        height="14"
                        width="14"
                      ...
                      <span
                        style="color:<?php echo esc_attr( sanitize_hex_color( $priceTags[0]['text_color'] ) ); ?>;"><?php echo esc_html($priceTags[0]['name']); ?></span>
                ...
                @if (!empty($bookingTag))
                    <div class="best-price-tag price-tag-item"
                         style="<?php echo !empty($bookingTag[0]['color']) ? 'background-color:' . esc_attr($bookingTag[0]['color']) . ';' : '' ?>">

                        <?php if (!empty($bookingTag[0]['icon_url'])) : ?>
                      <img
                        src="<?php echo esc_url($bookingTag[0]['icon_url']); ?>"
                        height="14"
                        width="14"
                      ...
                      <span
                        style="color:<?php echo esc_attr( sanitize_hex_color( $bookingTag[0]['text_color'] ) ); ?>;"><?php echo esc_html($bookingTag[0]['name']); ?></span>
          ...
          @if ( $bookNowUrl )
            <a
              href="{{ $bookNowUrl }}"
              @if ( $bookNowTarget === '_blank' ) target="_blank" rel="noopener noreferrer" @endif
              class="btn btn-orange w-full py-16! text-body-3! leading-16! text-white font-semibold rounded-lg uppercase mt-24 text-center"
            >{{ esc_html( $bookNowTitle ) }}</a>
          @else
            <button
              type="button"
              class="btn btn-orange w-full py-16! text-body-3! leading-16! text-white font-semibold rounded-lg uppercase mt-24"
            >{{ esc_html__( 'Book Now', 'fly-venture' ) }}</button>
          @endif
              ...
              @foreach ( $features as $feature )
                ...
                @if ( ! empty( $featureText ) )
                  <div
                    ...
                    @if ( $featureIconUrl )
                      <img src="{{ $featureIconUrl }}" alt="{{ $featureIconAlt }}" class="w-20 h-20">
                    @else
                      <img src="{{ esc_url( $location_check_uri ) }}" alt="" class="w-20 h-20" aria-hidden="true">
                    @endif
          ...
          <div class="calendar-widget w-full"></div>
          ...
          @if ( $ctaBtnUrl )
            <div class="btn-custom mt-25">
              <a href="{{ $ctaBtnUrl }}"
                 ...
                 aria-label="{{ esc_attr( $ctaBtnTitle ) }}">
